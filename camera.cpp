...
void camera::update()
{
	...
	case CAMERASTATE::SHAKE:
		...
		{
			_shakePivot.x = RND->getFromIntTo(_savePivot.x - _shakePower, _savePivot.x + _shakePower);
			_shakePivot.y = RND->getFromIntTo(_savePivot.y - _shakePower, _savePivot.y + _shakePower);
