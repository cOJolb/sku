...
#include "gameNode.h"
#include "mapTool.h"
POINT C_subWindow::ptMouse = POINT{ 0,0 };
CTRL C_subWindow::currentCTRL = CTRL::CTRL_DRAW;
C_subWindow::C_subWindow()
{
	backBuffer = new image();
	backBuffer->init(420, 520);
	fileCnt = 0;
}
...
void C_subWindow::release()
{
}

void C_subWindow::update()
{
...
void C_subWindow::render()
{
	HDC hdc = GetDC(hWnd);
	PatBlt(backBuffer->getMemDC(), 0, 0, 420, 520, WHITENESS);
...
void C_subWindow::SetScene(gameNode* scene)
{
	currentScene = scene;
	currentScene->init();
}

LRESULT C_subWindow::WndLogProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case  WM_COMMAND:
		...
		default:
			...
			case CTRL::CTRL_SAVE:
				...
				if (SUBWIN->getMapTool()->save(folderPath))
				{
					strcat(gameFolderPath, saveFileName);
					SUBWIN->getMapTool()->save(gameFolderPath);
			...
			case CTRL::CTRL_LOAD:
				...
				strcat(folderPath, saveFileName);
				if (SUBWIN->getMapTool()->load(folderPath))
...
void C_subWindow::CreateSubWindow()
{
	//로그 윈도우 생성
	int x, y, cx, cy;
	WNDCLASS wc;
	...
	wc.lpfnWndProc = (WNDPROC)C_subWindow::WndLogProc;
	wc.cbClsExtra = 0;
	wc.cbWndExtra = 0;
