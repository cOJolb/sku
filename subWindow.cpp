...
CTRL subWindow::currentCTRL = CTRL::CTRL_DRAW;
subWindow::subWindow()
{
...
LRESULT subWindow::WndLogProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case  WM_COMMAND:
		...
		default:
			...
			case CTRL::CTRL_LOAD:
				/*idx = SendMessage(SUBWINDOW->getListHandle(), LB_GETCURSEL, 0, 0);
				SendMessage(SUBWINDOW->getListHandle(), LB_GETTEXT, idx, (LPARAM)saveFileName);
				strcat(folderPath, saveFileName);
				if (SUBWINDOW->getMap()->load(folderPath))
			...
			case CTRL::CTRL_REMOVE:
				idx = SendMessage(SUBWINDOW->getListHandle(), LB_GETCURSEL, 0, 0);
				SendMessage(SUBWINDOW->getListHandle(), LB_GETTEXT, idx, (LPARAM)removeStr);
				if (idx != -1)
				{
					if (IDOK == MessageBox(SUBWINDOW->GetHwnd(), removeStr,
						"아래의 항목을 삭제하겠습니까?",
						MB_ICONQUESTION | MB_OKCANCEL))
					{
						SendMessage(SUBWINDOW->getListHandle(), LB_DELETESTRING, idx, 0);
			...
			case CTRL::CTRL_ROOM:
			case CTRL::CTRL_ITEM:
				SUBWINDOW->SetMonsterFrame({ 0, 0 });
				SUBWINDOW->SetObjFrame({ 0, 0 });
				SUBWINDOW->SetFrameIndex(LOWORD(wParam) - 9);
				break;
